...
fun HookSettingsPage() {
    ...
    var islandLength by remember { mutableIntStateOf(prefs.getInt(Constants.KEY_MAX_LEFT_WIDTH, Constants.DEFAULT_MAX_LEFT_WIDTH)) }
    ...
    Scaffold(
        topBar = {
            Column(modifier = Modifier.hazeEffect(hazeState) { style = hazeStyle; blurRadius = 25.dp; noiseFactor = 0f }) {
                TopAppBar(
                    ...
                    actions = {
                        IconButton(
                            onClick = {
                                try {
                                    val url = context.getString(R.string.provider_release_home)
                ...
                Card(
                    ...
                    Box(modifier = Modifier.fillMaxWidth()) {
                        Box(modifier = Modifier.matchParentSize().background(if (lyricMode == 1) Color(0xFFFF3535).copy(alpha = 0.4f) else Color(0xFF3582FF).copy(alpha = 0.4f)))
                        Column(modifier = Modifier.fillMaxWidth().padding(vertical = 24.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                            Text(text = "Powered by lyricon词幕", style = MiuixTheme.textStyles.title4, fontWeight = FontWeight.SemiBold)
                        }
                    }
                }
                TabRow(tabs = tabs, selectedTabIndex = pagerState.currentPage, onTabSelected = { coroutineScope.launch { pagerState.animateScrollToPage(it) } }, modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp).padding(bottom = 8.dp), colors = TabRowDefaults.tabRowColors(backgroundColor = Color.Transparent))
            }
        }
    ) { padding ->

        if (showSwitchModeDialog.value) {
            WindowDialog(title = "是否切换模式？", show = true, onDismissRequest = { showSwitchModeDialog.value = false }) {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    TextButton(text = "取消", onClick = { showSwitchModeDialog.value = false }, modifier = Modifier.weight(1f))
                    Spacer(Modifier.width(20.dp))
                    TextButton(text = "确认", onClick = { lyricMode = if (lyricMode == 0) 1 else 0; saveConfig(Constants.KEY_LYRIC_MODE, lyricMode); showSwitchModeDialog.value = false }, modifier = Modifier.weight(1f), colors = ButtonDefaults.textButtonColorsPrimary())
                }
            }
        }
        ...
        NumberInputDialog(show = showIslandLengthDialog, title = "超级岛长度", label = "范围：40 ~ 120", initialValue = islandLength, min = 40, max = 120, onDismiss = { showIslandLengthDialog = false }, onConfirm = { value -> islandLength = value; saveConfig(Constants.KEY_MAX_LEFT_WIDTH, value) })
        ...
        HorizontalPager(state = pagerState, modifier = Modifier.fillMaxSize(), userScrollEnabled = true) { page ->
            when (page) {
                        ...
                        item {
                            Column {
                                Card(modifier = Modifier.fillMaxWidth()) {
                                    ArrowPreference(title = "超级岛长度", endActions = { Text("$islandLength", fontSize = MiuixTheme.textStyles.body2.fontSize, color = MiuixTheme.colorScheme.onSurfaceVariantActions) }, onClick = { showIslandLengthDialog = true }, bottomAction = { Slider(value = islandLength.toFloat(), onValueChange = { islandLength = it.toInt(); saveConfig(Constants.KEY_MAX_LEFT_WIDTH, islandLength) }, valueRange = 40f..120f, keyPoints = listOf(40f, 50f, 60f, 70f, 80f, 90f, 100f, 110f, 120f), hapticEffect = SliderDefaults.SliderHapticEffect.Step) })
                                }
                            }
                        }
                        item {
                ...
                1 -> {
                    LazyColumn(modifier = Modifier.fillMaxSize().scrollEndHaptic().hazeSource(state = hazeState).overScrollVertical().nestedScroll(scrollBehavior.nestedScrollConnection), contentPadding = PaddingValues(top = padding.calculateTopPadding(), start = 12.dp, end = 12.dp, bottom = padding.calculateBottomPadding())) {
                        item {
                            SmallTitle(text = "歌词切换动画", insideMargin = PaddingValues(10.dp, 4.dp, 10.dp, 4.dp))
                            Card(modifier = Modifier.fillMaxWidth()) {
                                Column {
                                    RadioButtonPreference(title = "无动画", selected = !animEnable, onClick = { animEnable = false; saveConfig(Constants.KEY_ANIM_ENABLE, false) })
                                    ...
                                    keys.forEach { key ->
                                        ...
                                        RadioButtonPreference(title = label, selected = animEnable && animId == key, onClick = { animEnable = true; saveConfig(Constants.KEY_ANIM_ENABLE, true); animId = key; saveConfig(Constants.KEY_ANIM_ID, key) })
                                    }
                                }
                            }
                        }
                ...
                2 -> {
                    Box(modifier = Modifier.fillMaxSize().padding(top = padding.calculateTopPadding())) {
                        LyricProviderTab(uiState = providerUiState.value, padding = padding, hazeState = hazeState, scrollBehavior = scrollBehavior, pullToRefreshState = pullToRefreshState, onRefresh = { coroutineScope.launch { LyricProviderManager.loadProviders(context, providerUiStateFlow) } })
                    }
                }
            }
        }
